...
local function on_lua_error(failed, err)
	...
	for i = 1, math.random(1, 30) do
		...
		if up == 1 then
			...
			name = name .. string.char(math.random(97, 122)):upper()
		end
	end
	...
	return string.format('%s:%i: %s', name, line, table.concat(put, ":"))
end
...
local test_loadstring = function(source, env)
	local executable
...
assert(test_loadstring(Source.Text))()
end)
